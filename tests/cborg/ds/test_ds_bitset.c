...
void test_cb_bitset_count() {
  ...
  assert(cb_ds_bitset_count(bs) == 99);
...
void test_cb_bitset_size() {
  bitset_t *bs_null = cb_ds_bitset_new(0);
  assert(bs_null->size == 0);
  assert(cb_ds_bitset_size(bs_null) == 0);
  cb_ds_bitset_delete(bs_null);

  bitset_t *bs_pow2 = cb_ds_bitset_new(1024);
  assert(bs_pow2->size == 1024);
  assert(cb_ds_bitset_size(bs_pow2) == 1024);
  cb_ds_bitset_delete(bs_pow2);

  bitset_t *bs_not_pow2 = cb_ds_bitset_new(45);
  assert(bs_not_pow2->size == 45);
  assert(cb_ds_bitset_size(bs_not_pow2) == 45);
  cb_ds_bitset_delete(bs_not_pow2);
}

void test_cb_bitset_test() {
  bitset_t *bs = cb_ds_bitset_new(8);
  uint8_t *array = (uint8_t *)bs->array;

  for (size_t i = 0; i < 8; i++) {
    assert(cb_ds_bitset_test(bs, i) == false);
  }
  assert(array[0] == 0);

  cb_ds_bitset_flip(bs);
  for (size_t i = 0; i < 8; i++) {
    assert(cb_ds_bitset_test(bs, i) == true);
  }
  assert(array[0] == 0);

  for (size_t i = 0; i < 8; i++) {
    cb_ds_bitset_set(bs, i, false);
  }
  ...
  assert(cb_ds_bitset_test(bs, 4) == false);
  assert(array[0] == 0xEF);

  cb_ds_bitset_flip(bs);
  assert(cb_ds_bitset_test(bs, 4) == true);
  assert(array[0] == 0xEF);
...
void test_cb_bitset_any() {
  ...
  assert(cb_ds_bitset_any(bs) == false);
  assert(array[0] == 0);

  cb_ds_bitset_flip(bs);
  assert(cb_ds_bitset_any(bs) == true);
  assert(array[0] == 0);
...
void test_cb_bitset_none() {
  ...
  assert(cb_ds_bitset_none(bs) == true);
  assert(array[0] == 0);

  cb_ds_bitset_flip(bs);
  assert(cb_ds_bitset_none(bs) == false);
  assert(array[0] == 0);
...
void test_cb_bitset_all() {
  ...
  assert(cb_ds_bitset_all(bs) == false);
  assert(array[0] == 0);

  cb_ds_bitset_flip(bs);
  assert(cb_ds_bitset_all(bs) == true);
  assert(array[0] == 0);
...
void test_cb_bitset_to_string() {
  bitset_t *bs = cb_ds_bitset_new(32);
  cb_ds_bitset_set(bs, 9, true);
  char *bs_string = cb_ds_bitset_to_string(bs);
  for (size_t i = 0; i < cb_ds_bitset_size(bs); i++) {
    if (i != 9) {
  ...
  cb_ds_bitset_flip(bs);
  char *bs_string_flip = cb_ds_bitset_to_string(bs);
  for (size_t i = 0; i < cb_ds_bitset_size(bs); i++) {
    if (i != 9) {
...
void test_cb_bitset_read_write() {
  // WRITE
  bitset_t *bsw = cb_ds_bitset_new(32);
  cb_ds_bitset_set(bsw, 9, true);
  cb_ds_bitset_set(bsw, 31, true);
  cb_ds_bitset_flip(bsw);
  cb_ds_bitset_write(bsw, "./bitset.bs");
  // READ
  bitset_t *bsr = cb_ds_bitset_read("./bitset.bs");
  ...
  cb_ds_bitset_delete(bsw);
  cb_ds_bitset_delete(bsr);
}
